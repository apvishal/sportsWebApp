...
exports.verifySport = (req,res,next) => {
    if (! (["mlb", "nba", "mls", "nfl"].includes(req.params.sport))) {
        return next(new Error(`${req.params.sport} is not a defined sport in this webapp...`));
    }
    next();
};
...
exports.getTeam = async (req,res,next) => {
    try {
        ...
        res.status(404).json({status: 'failure', message: err.stack});
    }
};
